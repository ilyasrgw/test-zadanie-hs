...
function getOptionList(navigationTree) {
  return navigationTree.reduce((optionTree, navItem) => {
    optionTree.push(navItem);
    if (Array.isArray(navItem.submenu) && navItem.submenu.length > 0) {
      optionTree = optionTree.concat(getOptionList(navItem.submenu));
    }
    return optionTree;
  }, []);
}
